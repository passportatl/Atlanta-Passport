...
**Exception — event bonus stamps:** events do NOT use this dual-layer flow. Toggling `isBonusStamp` on an event (Event Hub checkbox or edit drawer) auto-upserts a companion businesses row (`event-<slug>`, category "events") via the events PATCH route; toggling off deactivates it (never deletes, so collected stamps survive). If the event has no slug, the route persists a stable one (`<name>-<id8>`) at enable time so renames still address the same row. Never hand-seed event bonus stamps.
