...
function GoogleGlyph() {
  return (
    <svg className="h-5 w-5" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <path
        fill="#EA4335"
        d="M12 4.75c1.77 0 3.35.61 4.6 1.8l3.43-3.43C17.95 1.19 15.23 0 12 0A12 12 0 0 0 1.27 6.62l4 3.09C6.22 6.86 8.87 4.75 12 4.75Z"
      />
    </svg>
  );
}
...
export function SocialAuthButtons({
  ...
  const start = async (provider: Provider) => {
    ...
    if (!acceptTerms || !acceptPrivacy) {
      setError(
        "Accept the Terms of Service and Privacy Policy before continuing.",
      );
      return;
    }
    setError(null);
    ...
    try {
      await signUp.update({
        unsafeMetadata: { acceptTerms, acceptPrivacy, marketingOptIn },
      });
