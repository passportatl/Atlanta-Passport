...
export default function Listing() {
  ...
  const { locations: exploreLocations, status: exploreDataStatus } =
    useExploreLocations();
  const publicLocation = exploreLocations.find(
    (location) =>
      location.id === id ||
      location.canonicalId === id ||
      location.canonicalSlug === id,
  );
      ...
      <div className="min-h-[60vh] flex items-center justify-center p-4 bg-muted/20">
        <div className="card-pop bg-card max-w-md w-full p-8 text-center">
          ...
          <Link
            ...
            className="button-pop inline-flex items-center justify-center gap-2 w-full"
          >
  ...
  if (exploreDataStatus === "loading") {
    return (
      <div className="min-h-[60vh] flex items-center justify-center p-4 bg-muted/20">
        <div
          role="status"
          className="card-pop bg-card max-w-md w-full p-8 text-center"
        >
          <h2 className="text-2xl font-serif font-bold text-foreground mb-4">
            Checking listing access
          </h2>
          <p className="text-muted-foreground">
            Passport ATL is confirming the current listing details.
          </p>
        </div>
      </div>
    );
  ...
  if (!publicLocation?.detailPageEnabled) {
    return (
      <div className="min-h-[60vh] flex items-center justify-center p-4 bg-muted/20">
        <div className="card-pop bg-card max-w-md w-full p-8 text-center">
          ...
          <p className="text-sm font-display uppercase tracking-wider text-brand-red mb-4">
            {business.neighborhood}
          </p>
          <p className="text-muted-foreground mb-8">
            This location currently has a basic Passport ATL listing. Full
            location details are not available at this time.
          </p>
          <Link
            href="/passport/explore"
            className="button-pop inline-flex items-center justify-center gap-2 w-full"
    ...
    <div className="w-full pb-24 bg-background" data-location={business.id}>
      ...
      <div className="w-full h-[40vh] md:h-[50vh] relative">
        ...
        <div className="absolute bottom-6 md:bottom-8 left-6 md:left-12 lg:left-24 right-28 md:right-32 text-white">
          ...
          <div className="hidden md:block">
            <h1 className="text-5xl lg:text-7xl font-serif font-bold mb-3">
              {business.name}
            </h1>
      ...
      <div className="container mx-auto px-4 md:px-12 lg:px-24 -mt-6 md:mt-12 relative z-10">
          ...
          (() => {
            ...
            return (
              <div
                className={
                  ac
                    ? "rounded-2xl p-6 md:p-8 mb-12 shadow-sm relative overflow-hidden border-2"
                    : "bg-accent/10 border-2 border-accent/30 rounded-2xl p-6 md:p-8 mb-12 shadow-sm relative overflow-hidden"
                }
                style={
                  ac
                    ? { backgroundColor: `${ac}1a`, borderColor: `${ac}88` }
                    : undefined
                }
              >
                ...
                <div className="relative z-10">
                  <div className="flex items-center mb-3">
                    <div
                      className={
                        ac
                          ...
                          : "bg-accent text-accent-foreground w-8 h-8 rounded-full flex items-center justify-center mr-3 shadow-md"
                      }
                      style={
                        ac ? { backgroundColor: ac, color: fg } : undefined
                      }
                    >
                    ...
                    <h3
                      className={
                        ac
                          ? "font-bold tracking-wide uppercase text-sm text-foreground"
                          : "font-bold text-accent tracking-wide uppercase text-sm"
                      }
                    >
            ...
            {branding?.showComingSoonPanel && (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Exclusive partner features — custom gallery, loyalty perks,
                  and event listings — are being prepared for this location.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="space-y-10 mt-10">
              ...
              {business.hours && (
                <div>
                  <div className="flex items-center text-primary font-bold mb-2">
                    ...
                    {t("listing_page.hours_label")}
                  </div>
                  <p className="text-muted-foreground whitespace-pre-line">
                    {business.hours
                      .split(/(Book in advance)/g)
                      .map((part, i) =>
              ...
              {(business.address ||
                ...
                <div className="card-pop bg-card p-6">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h4 className="font-display text-xs tracking-[0.18em] text-foreground uppercase">
                      {t("listing_page.address_label")}
                    </h4>
                  </div>
                  {business.address && (
                  ...
                  <p className="text-sm text-muted-foreground mb-5">
                    {business.neighborhood} · Atlanta, GA
                  </p>
                  {business.lat != null && business.lng != null && (
                    <MapSnapshot
                      lat={business.lat}
                      lng={business.lng}
                      name={business.name}
                      isStamp={Boolean(STAMP_SLUG[business.id])}
                      customMarkerSrc={branding?.mapMarkerSrc}
                    />
                  )}
                  <a
                  ...
                  >
                    <Navigation className="w-4 h-4" />{" "}
                    {t("listing_page.directions_label")}
                  </a>
                </div>
              )}
            </div>
                ...
                {lc.featuredExperiences.map((exp) => (
                  <li key={exp.name} className="flex gap-4">
                    <div
                      ...
                      style={{
                        backgroundColor:
                          branding?.accentColor ?? "hsl(var(--brand-red))",
                        minHeight: "1.5rem",
                      }}
                      aria-hidden
                    />
                  ...
                  <ul className="space-y-3 text-muted-foreground leading-relaxed">
                    {business.transit?.marta && (
                      <li className="flex gap-3">
                        <span className="badge-sticker bg-brand-navy text-brand-cream text-[10px] shrink-0 self-start mt-0.5">
                          MARTA
                        </span>
                    ...
                    {business.transit?.beltline && (
                      <li className="flex gap-3">
                        <span className="badge-sticker bg-brand-lime text-foreground text-[10px] shrink-0 self-start mt-0.5">
                          BELTLINE
                        </span>
              ...
              {(lc?.accessibilityNote ||
                ...
                <section className="pt-6 border-t border-border">
                  <div
                    ...
                    style={accentTextStyle}
                  >
                    <ShieldCheck className="w-5 h-5 mr-2" /> Accessibility &
                    Policies
                  </div>
                  {lc?.accessibilityNote && (
          ...
          {business.menu && business.menu.length > 0 && (
            <section className="pt-6 border-t border-border">
              ...
              <p className="text-xs text-muted-foreground mt-4 italic">
                Menu may rotate — call ahead for daily specials.
              </p>
            </section>
          )}
          ...
          {branding?.showEventCalendar &&
            branding.venueNames &&
            branding.venueNames.length > 0 && (
