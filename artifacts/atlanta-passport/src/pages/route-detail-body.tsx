...
// Shared route-detail content used by both the marketing `/routes/:id` page and
// the in-shell `/passport/routes/:id` view. Start/time are controlled so the
// in-shell version can drive the same options that highlight the map route;
// `hrefBase` controls where prev/next link.
export default function RouteDetailBody({
  id,
  hrefBase,
  start,
  time,
