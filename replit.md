...
- **Map routes** (`mapRoutes` in `src/data/sample-data.ts`): curated routes built from real listed spots. Every route carries `byTime: {morning, noon, night}` ordered `businessId` lists (stops AND order change by time of day — afternoon-only anchors like the Trap Museum, Nakato, and Varasano's are saved for last in the morning and lead the night) plus `starts: {marta, parking}`. `resolveRoute(route, start, time)` picks the time-of-day stop list, reverses it when `start==="parking"`, computes per-leg haversine distances, and sizes the day to ~4–6 hours via **walking minutes + per-stop average visit time** (`visitMinutes` on a business, else `VISIT_MINUTES_BY_CATEGORY` default). It returns `miles` (summed from legs), `duration`, `legs[]`, and `visits[]` (per-stop minutes, rendered as `~Xm` in `RoutesFeed`). On `/passport/routes`, clicking a `RoutesFeed` card sets `selectedRouteId` in MapShell → the shared map shows ONLY that route's stops (ordered) and `BusinessMap` draws a yellow `RoutePath` polyline connecting them + fits bounds; clicking the active card again clears it (map shows all spots). With no route selected the map shows all businesses. `BusinessMap` accepts an optional `routePath?: {lat,lng}[]` prop.
