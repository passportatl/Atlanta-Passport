...
**Home page order (redesigned — "Field Guide / route-journey" sales tool):** Marquee → **Hero** (centered typographic "The Unofficial Field Guide to the Real Atlanta" with floating polaroid photos + an animated passport stamp; CTAs "Get the Passport" → `/passport/explore` and "See the Route" → `/beltline`; parallax via `useScroll`) → **Pillars** ("Your ticket to the city" — zigzag route-line layout of the 5 value props: Exclusive Deals, Match-Week Events, Zero Planning, Real Neighborhoods, Bike & Walk Routes) → **Journey** (navy horizontal-scroll cards of curated `routes`, showing stops/miles) → **Spots & Events** (orange grid mixing featured `events` + `businesses` with `offer` perks) → **Business CTA** (red block, "Get Listed" → `/apply`) → Footer. Each home section is its own component in `home.tsx` (`HeroSection`/`PillarsSection`/`JourneySection`/`SpotsAndEventsSection`/`BusinessCtaSection`). All visible copy (incl. decorative stamp/polaroid labels) lives under `home.*` in the 9 locale files; pillar circle colors use an explicit `bg`/`fg` class map (never `bg-brand-${var}` — Tailwind can't see dynamic class names).
