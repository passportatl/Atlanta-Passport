...
- Routing via `wouter` with base path `import.meta.env.BASE_URL`. App.tsx splits routing by URL prefix: `/stamp/:slug` (full-bleed), `/admin/stamps` (admin), the MapShell routes (see below — Router returns null, checked BEFORE the `/passport` branch), `/passport` (PassportLayout w/ bottom nav — profile home only), everything else (marketing Layout w/ Navbar+Footer).
